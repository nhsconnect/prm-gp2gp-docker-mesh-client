...
from gp2gpmi.miuploader import MIToS3Uploader
...
def test_uploads_file():
    ...
    uploader = MIToS3Uploader(mock_mesh_inbox_scanner, mock_file_registry, mock_file_uploader)
...
def test_uploads_multiple_files():
    ...
    calls = [call(mock_mesh_file, "fake-bucket"),
             call(mock_second_mesh_file, "fake-bucket")]
...
def test_only_uploads_new_files():
    ...
    mock_mesh_inbox_scanner = MagicMock()
    mock_file_registry = mock_registry(already_processed={
        (mock_old_file, "fake-bucket")
    })
    mock_file_uploader = MagicMock()
    ...
    mock_file_uploader.upload.assert_called_once_with(MeshFile(path='path/to/newfile.dat', date_delivered=A_DATE), "fake-bucket")
